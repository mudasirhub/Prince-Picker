...
```bash
node "M:/Prince-Picker-main/.claude/get-shit-done/bin/gsd-tools.cjs" validate health $REPAIR_FLAG
```
...
```bash
node "M:/Prince-Picker-main/.claude/get-shit-done/bin/gsd-tools.cjs" validate health
```
...
TASKS_DIR="M:/Prince-Picker-main/.claude/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in M:/Prince-Picker-main/.claude/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf M:/Prince-Picker-main/.claude/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
