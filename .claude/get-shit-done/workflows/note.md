...
- **Global scope**: `M:/Prince-Picker-main/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `M:/Prince-Picker-main/.claude/notes/`)
...
2. Glob `M:/Prince-Picker-main/.claude/notes/*.md` (if directory exists) — global notes
...
Global (M:/Prince-Picker-main/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `M:/Prince-Picker-main/.claude/notes/` — works in any directory
