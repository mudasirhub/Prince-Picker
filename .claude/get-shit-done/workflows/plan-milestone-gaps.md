...
HIGHEST=$(node "M:/Prince-Picker-main/.claude/get-shit-done/bin/gsd-tools.cjs" phases list --pick directories[-1])
```
...
```bash
node "M:/Prince-Picker-main/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
