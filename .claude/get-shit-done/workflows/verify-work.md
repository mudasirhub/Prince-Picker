...
<template>
@M:/Prince-Picker-main/.claude/get-shit-done/templates/UAT.md
</template>
...
```bash
INIT=$(node "M:/Prince-Picker-main/.claude/get-shit-done/bin/gsd-tools.cjs" init verify-work "${PHASE_ARG}")
...
```
UI_PHASE_FLAG=$(node "M:/Prince-Picker-main/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase --raw 2>/dev/null || echo "true")
...
```bash
CHECKPOINT=$(node "M:/Prince-Picker-main/.claude/get-shit-done/bin/gsd-tools.cjs" uat render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
node "M:/Prince-Picker-main/.claude/get-shit-done/bin/gsd-tools.cjs" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
- Follow @M:/Prince-Picker-main/.claude/get-shit-done/workflows/diagnose-issues.md
